...
namespace MALFUNCTIONING
{
    ...
    IntPtr lpParam);
        ...
        {
            // 窗口样式
            public const uint WS_CHILD = 0x40000000;        // 子窗口（依附父窗口）
            public const uint WS_VISIBLE = 0x10000000;      // 可见
            public const uint WS_BORDER = 0x00800000;

            // CheckBox专属按钮样式
            public const uint BS_CHECKBOX = 0x00000002;     // 标准复选框
            public const uint BS_AUTOCHECKBOX = 0x00000003; // 自动切换勾选（推荐）

            // 消息ID
            public const uint BM_GETCHECK = 0x00F0;         // 获取勾选状态
            public const uint BM_SETCHECK = 0x00F1;         // 设置勾选状态
            public const uint BN_CLICKED = 0x0000;          // 按钮点击通知消息

            // 勾选状态返回值
            public const int BST_UNCHECKED = 0;     // 未勾选
            public const int BST_CHECKED = 1;       // 已勾选
            public const int BST_INDETERMINATE = 2; // 不确定（三态）
